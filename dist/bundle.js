(() => {
  ...
  // src/config.ts
  console.log("Fichier de config API charg\xE9");
  var API_URL = "https://webetu.iutnc.univ-lorraine.fr/www/canals5/phox/api";

  // src/photoloader.ts
  console.log("Fichier photoloader charg\xE9");
  ...
  function loadResource(uri2) {
    return fetch("https://webetu.iutnc.univ-lorraine.fr" + uri2, {
      credentials: "include"
    }).then((response) => response.json()).catch((error) => {
      console.log(error);
      throw error;
  ...
  function displayCategory(category) {
    ...
    if (zone) {
      zone.textContent = "categorie : " + category.categorie.nom;
    }
  }
  ...
  // src/gallery.ts
  console.log("gallery.ts charg\xE9");
  var uri = "/www/canals5/phox/api/photos";
  var currentGallery = null;
  function load(url = uri) {
    return loadResource(url).then((gallery) => {
      currentGallery = gallery;
      return gallery;
    });
  }
  function nextPhoto() {
    return load(currentGallery.links.next.href);
  }
  function prevPhoto() {
    return load(currentGallery.links.prev.href);
  }
  function firstPhoto() {
    return load(currentGallery.links.first.href);
  }
  function lastPhoto() {
    return load(currentGallery.links.last.href);
  }

  // src/galleryui.ts
  var import_handlebars2 = __toESM(require_handlebars());
  function displayGalerie(gallery) {
    const templateElement = document.querySelector("#galleryTemplate");
    const container = document.querySelector("#gallery_container");
    const template = import_handlebars2.default.compile(templateElement.innerHTML);
    ...
    images.forEach((img, index) => {
      img.addEventListener("click", () => {
        ...
        loadResource(href).then((photo) => {
          displayPicture(photo);
          loadResource(photo.links.categorie.href).then((category) => {
            displayCategory(category);
          });
          loadResource(photo.links.comments.href).then((comments) => {
  ...
  // src/index.ts
  console.log("fichier index charg\xE9");
